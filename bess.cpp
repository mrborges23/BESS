...
//global variables
int M, iterations, n_chains, sample_freq, na_muji, na_N, na_phij, L;
...
int main(int argc, char * argv[]) {
  ...
  */
  ...
  if (model=="N") {
    std::cout << "\n\n  BESS\n\n  Parameters were succefully read.\n";
    std::cout << "  Model: "<< model << "\n  Mutation rate: "  << mu << "\n  Number of generations: " << iterations << "\n  Number of chains: " << n_chains << "\n  Sample MCMC frequency: " << sample_freq << "\n\n";
    n_MCMC(str_sfile);
    std::cout << "\n\n  BESS has finished!\n  The log file has been written.\n\n";
...
void ReadControlFile(const std::string str_control_file, std::string& model, std::string& str_sfile) {
  ...
  // updates parameters
  cfile >> word >> model >> word >> str_sfile >> word >> mu >> word >> m_N >> word >> v_N >> word >> m_phij >> word >> v_phij >> word >> iterations >> word >> n_chains >> word >> sample_freq >> word >> L; 
  cfile.close();
...
void s_MetropolisHastings_muji(double& muij, double& muji, double& phij, int& N){
  ...
  //std::cout << "muji: " << muji << " / muji1: " << muji1 << " : muij1: " << muij1 << " | " << tunning_muji << "," << na_muji << "\n";
  ...
  //std::cout << "lnl: " << lnL << "lnl1: " << lnL1 << " AP: " << acceptance_prob <<  " \n";
...
void s_MetropolisHastings_phij(double& muij, double& muji, double& phij, int& N){
  ...
  double muij1 = muji*mu/( muji*(1.0+pow(phij1,N-1))-pow(phij1,N-1)*mu );  

  if ( (phij1 < 1.0) || (muij1< 0.0) || (muij1>1.0)  ){//|| ( (phij1-1.0) >1.0/N) ) {
    return; 
  }
  
  // Likelihood
  double lnL1 = s_Likelihood(muij1,muji,phij1,N);
...
void s_MetropolisHastings_N(double& muij, double& muji, double& phij, int& N){
  ...
  double muij1 = muji*mu/( muji*(1.0+pow(phij,N1-1))-pow(phij,N1-1)*mu );  

  if ( (N1<M) || (N1==N) || (muij1< 0.0) || (muij1>1.0) ) {
    return; 
  }
  
  // Likelihood
  double lnL1 = s_Likelihood(muij1,muji,phij,N1);
...
void s_MCMC(std::string& str_ifile){
  ...
  for (int c=0; c<n_chains; ++c){
    ...
    // preliminaries
    ofile << "gen\tlnL\tmuji\tmuij\tsigmaj\tN\n"; 
    ...
    int N       = m_N;
    double phij = 1.0+1.0/(2.0*m_N); // rexp(N);
    ...
    na_phij = 0; 
    na_N = 0;
    tunning_muji = muij/10000;
    tunning_phij = phij-1.0;
    tunning_N    = v_N;
    ...
    for (int i=1; i<(iterations+1); ++i){
      ...
      if (i%sample_freq == 0) {
        ...
        std::ostringstream os_muij,os_muji,os_phij,os_lnL,os_tunning_muji,os_tunning_phij; 
        os_muij << muij;
        os_muji << muji;
        os_phij << phij-1.0;
        os_lnL  << lnL;
        ...
        ofile << i << "\t" << os_lnL.str() << "\t" << os_muji.str() << "\t" << os_muij.str() << "\t" << os_phij.str() << "\t" << std::to_string(N) << "\n"; 
        //ckfile << i << "\t" << os_muij.str() << "\t" << os_muji.str() << "\t" << os_phij.str() << std::to_string(N) << "\t" << std::to_string(na_muji) << "\t" <<  std::to_string(na_phij) << "\t" << std::to_string(na_N) << "\t" << os_tunning_muji.str() << "\t" << os_tunning_phij.str() << "\t" << std::to_string(tunning_N) << "\n"; 
      
        //output
        //std::cout << i+1 << "\t" << lnL << "\t" << muji << "\t" << muij << "\t" << 1.0-phij << "\t" << N << "\n";
